...
const FolderSelect = ({setdownloadModal}) => {
    const [folders, setfolders] = useState(['folder name 1','folder name 2','folder name 3','fodler name 4','folder name 5','folder name 6','folder name 7','folder name 8'])
    const uploadFile=()=>{
        //upload files;
        setdownloadModal(false);
    }
    return (
    <div className='fixed  bg-black transition ease-in-out delay-300 inset-0 bg-opacity-50 backdrop-blur-sm flex justify-center items-center'>
    <div className='w-4/5 sm:w-1/2 h-1/2 sm:h-4/6 transition ease-in-out delay-100 radius relative bg-black  border-dashed border-white border-[1px] flex justify-center items-center rounded-3xl '>
            <button onClick={()=>{setdownloadModal(false)}} className='absolute p-2 top-4 right-4'>
            ...
            <div className='grid grid-cols-2 mt-5 h-full overflow-auto w-full'>
                <button className=' flex flex-col items-center justify-center border-[1px] border-gray-400 bg-[#1e1e1e] px-5 text-lg rounded-xl py-2 m-3 '>
